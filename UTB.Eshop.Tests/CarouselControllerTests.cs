...
namespace UTB.Eshop.Tests
{
    public class CarouselControllerTests
    {
        ...
        public async Task CarouselCreate_InvalidFailure()
        {
            ...
            var fileCheckContent = new Mock<ICheckFileContent>();
            fileCheckContent.Setup(fcc => fcc.CheckFileContent(It.IsAny<IFormFile>(), It.IsAny<string>())).Returns(false); // change to false

            var fileCheckLength = new Mock<ICheckFileLength>();
            fileCheckLength.Setup(fcl => fcl.CheckFileLength(It.IsAny<IFormFile>(), It.IsAny<long>())).Returns(false); // change to false
